...
import com.spring.enums.UserRole;
...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    List<Account> findAllByRole(UserRole userRoleEnum);
    ...
    List<Account> fetchAllAccountOrderByAge();

    // ------------------- Native QUERIES ------------------- //

    //Write a native query to read all accounts with an age lower than a specific value
    @Query(value=" SELECT *  FROM account_details where age < ?1  ", nativeQuery = true)
    List<Account> fetchAllLessThan(int age);
    ...
    List<Account> fetchAllContains(@Param("str") String str);
